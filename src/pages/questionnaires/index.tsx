...
type ColumnType = ColumnDef<QuestionnaireInterface, unknown>;
...
export function QuestionnaireListPage(props: QuestionnaireListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getQuestionnaires({
        relations: ['user', 'requirement.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<QuestionnaireInterface>>(
    () => `/questionnaires?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: QuestionnaireInterface) => {
    if (hasAccess('questionnaire', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/questionnaires/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('requirement', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.requirement,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/questionnaires/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="questionnaire" />
        </Flex>
