...
import { getChatbots } from 'apiSdk/chatbots';
import { ChatbotInterface } from 'interfaces/chatbot';
import { getRequirements } from 'apiSdk/requirements';
import { RequirementInterface } from 'interfaces/requirement';
import { getIterations } from 'apiSdk/iterations';
import { IterationInterface } from 'interfaces/iteration';

import { IterationListPage as ShowCaseList } from 'pages/iterations';

function DashboardPage() {
  const { session } = useSession();

  const chatbotStat = useSWR<PaginatedInterface<ChatbotInterface>>(
    () => `/chatbots`,
    () => getChatbots({ limit: 1, offset: 0 }),
  );

  const requirementStat = useSWR<PaginatedInterface<RequirementInterface>>(
    () => `/requirements`,
    () => getRequirements({ limit: 1, offset: 0 }),
  );

  const iterationStat = useSWR<PaginatedInterface<IterationInterface>>(
    () => `/iterations`,
    () => getIterations({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Chatbots',
      result: chatbotStat?.data?.totalCount,
      isLoading: chatbotStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Requirements',
      result: requirementStat?.data?.totalCount,
      isLoading: requirementStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Iterations',
      result: iterationStat?.data?.totalCount,
      isLoading: iterationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`iterations`} passHref legacyBehavior>
                  <Button
